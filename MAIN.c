...
void MAIN_vInit(void)
{
 ...
 // SCU_RSTCON1 |= 0x0080U;         /*WDTI ->App& Inter App Reset */
  SCU_RSTCON1 |= 0x00C0U;         /*WDTI ->Inter App Reset[Dont Reset PORT State] */
  //SCU_RSTCON1   &=~ 0x00C0U;    /*Don't Generate WDT Reset*/
...
#define BOOTPSRAM_START &Boot_Function
#define APP_FLASH_START	0xC04000		/*JMP to APP RESET*/



#define Boot_Main_Ptr 0xE00000 
#define SCU_POS_CPUCON1_VECSC     5U
#define SCU_VECSEG_FLASH 0xC0
#define SCU_VECSEG_PSRAM 0xE0
#define SCU_CODE_PSRAM_FLASH_START 0xC40000
#define SCU_CODE_PSRAM_FLASH_END   0xC42000

void Scu_CopyWords(unsigned int SHUGE *DestPtr,
...
void main(void)
{
  ...
  /*以下代码将FLASH程序复制到PSRAM*/
	hmemcpy (SROM_PS_TRG(Bootroom), SROM_PS_SRC(Bootroom), SROM_PS_LEN(Bootroom));
  	hmemcpy (SROM_PS_TRG(PFLASH), SROM_PS_SRC(PFLASH), SROM_PS_LEN(PFLASH));		
 // Scu_CopyVectorToPsram(0xE0);			/*中断向量表可以不切换*/
  	JMP(BOOTPSRAM_START);					/*跳到PSRAM里执行程序*/
 // JMP(APP_FLASH_START);
...
void Scu_CopyVectorToPsram(unsigned int Vector)
{
  unsigned int SHUGE *SrcPtr;
  unsigned int SHUGE *DestPtr;
  unsigned int ScalingFactor;
  unsigned int Words;
  unsigned int Offset;

  /* get scaling factor for vectors from CPUCON1 */
  ScalingFactor = ((unsigned int)CPUCON1 >> SCU_POS_CPUCON1_VECSC) + 1;

  /* calculate number of words for vectors */
  Words = 1U << ScalingFactor;

  /* calculate address offset of vector in bytes */
  Offset = Vector << (ScalingFactor + 1);

  /* set start address of vector in flash */
  SrcPtr = MKSP(Offset, SCU_VECSEG_FLASH);

  /* set start address of vector in PSRAM */
  DestPtr = MKSP(Offset, SCU_VECSEG_PSRAM);

  /* copy words from flash to PSRAM */
  Scu_CopyWords(DestPtr, SrcPtr, Words);

} /* end of function Scu_CopyVectorToPsram */
...
void Main_App(void)
{
	...
	while (1)
	{
		...
		TOGGLEIO_P10_3;
		App_Delay();
	  if(Boot_App_Switch_u08 == 0xB5)
	  {
	 	 JMP(BOOTPSRAM_START);									/*跳到 PSRAM FUNCTION*/
	  }
	}
